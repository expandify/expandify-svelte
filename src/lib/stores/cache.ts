...
export namespace Cache {
	...
	export const enum State {
		...
		Finished = 'Finished',
		// Indicates that an error occured will loading or otherwise
		Error = 'Error'
	}
	...
	export function setTotalItems<T extends SpotifyData>(
		cache: Writable<Data<T>>,
		totalItems: number
	) {
	...
	export function setLoadedItems<T extends SpotifyData>(
		cache: Writable<Data<T>>,
		loadedItems: number
	) {
...
if (browser) {
	...
	// Saveing everything will exceed the quota of the sessionStorage (ca. 5MB)
	// The same happens in localStorage
