...
export async function reloadFollowedArtists() {
	try {
		...
		const func = async (api: SpotifyWebApi.SpotifyWebApiJs, after: string | null) =>
			(await api.getFollowedArtists({ limit: 10, ...(after && { after: after }) })).artists;
		const artists = (
			await makeCursorRequest(func, (cursor) => {
		...
		Cache.setState(Cache.artists, Cache.State.Error);
		console.error(err);
		throw error(500, "Error on loading Artists.")
	}
}
