...
class SpotifyPersistence {
    ...
    async loadSavedAlbums() {
        ...
        albums.stopLoading();
        if (!get(albums).error) {
            notifications.addSuccess("Finished loading albums")
        }
    }
    ...
    async loadFollowedArtists() {
        ...
        artists.stopLoading();
        if (!get(artists).error) {
            notifications.addSuccess("Finished loading artists")
        }
    }
    ...
    async loadPlaylists() {
        ...
        playlists.stopLoading();
        if (!get(playlists).error) {
            notifications.addSuccess("Finished loading playlists")
        }
    }
    ...
    async loadSavedTracks() {
        ...
        tracks.stopLoading();
        if (!get(tracks).error) {
            notifications.addSuccess("Finished loading tracks")
        }
    }
    ...
    async loadUser() {
        ...
        user.stopLoading();
        if (!get(user).error) {
            notifications.addSuccess("Finished loading user")
        }
    }
